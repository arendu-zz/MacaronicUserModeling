...
def batch_sgd_accumilate(result):
    global f_en_en_theta, f_en_de_theta
...
if __name__ == '__main__':
    ...
    all_training_instances = training_instances[:]
    ...
    for epoch in range(2):
        ...
        for ti in all_training_instances:
            pool.apply_async(batch_sgd, args=(
                ...
                en_domain,
                de2id,
                en2id), callback=batch_sgd_accumilate)
        pool.close()
        pool.join()
        '''batch_sgd(training_instance=training_instance,
                  f_en_en_theta=f_en_en_theta,
                  f_en_de_theta=f_en_de_theta,
                  phi_en_en=phi_en_en,
                  phi_en_de=phi_en_de,
                  en_domain=en_domain,
                  de2id=de2id,
