...
def batch_sgd(training_instance,
    ...
    if len(fg.variables) > 10:
        ...
        print 'dif_cgtimes:', np.sum(fg.diff_cg_times) / len(fg.diff_cg_times), 'total', np.sum(
            ...
            fg.exp_cg_times)
        print 'num vars   :', len(fg.variables)
    p = fg.get_posterior_probs()
...
if __name__ == '__main__':
    ...
    if mode == 'training':
        ...
        f_en_en_theta = np.ones((1, len(f_en_en_names)))
        f_en_de_names = ['ed', 'ped', 'full_history', 'hit_history']
        f_en_de_theta = np.ones((1, len(f_en_de_names)))
        domain2theta = {}
        for d in domains:
    ...
    if mode == 'training':
        for epoch in range(1):
            ...
            for ti in training_instances:
                [sent_id, p, g_en_en, g_en_de, sample_ag] = batch_sgd(
                    ti,
                    f_en_en_names,
                    f_en_de_names,
                    f_en_en_theta,
