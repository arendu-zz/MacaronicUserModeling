...
if __name__ == '__main__':
    ...
    test_instances = training_instances[40:50]
    all_training_instances = training_instances[1330:1335]
    lr = 0.1
    for epoch in range(3):
        ...
        for t_idx, training_instance in enumerate(all_training_instances):
            ...
            print '\tcell grad time per factor:', sum(fg.cg_times) / float(len(fg.cg_times)), len(fg.cg_times)
            print '\tspar grad time per factor:', sum(fg.sgg_times) / float(len(fg.sgg_times)), len(fg.sgg_times)
            print '\tgrad grad time per factor:', sum(fg.gg_times) / float(len(fg.gg_times)), len(fg.gg_times)
        ...
        print 'ave  grad time pf:', sum(grad_times_per_factor) / float(len(grad_times_per_factor))
